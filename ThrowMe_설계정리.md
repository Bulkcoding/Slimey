...
- `src/ThrowMe/ThrowMe.csproj`
- `src/ThrowMe/app.manifest`
- `src/ThrowMe/App.xaml` / `App.xaml.cs`
- `src/ThrowMe/Views/SlimeWindow.xaml` / `SlimeWindow.xaml.cs`
- `src/ThrowMe/Models/AppSettings.cs`

**Phase 2 (단일 모니터 물리: 투척 속도 + 관성 + 반사 + 마찰/정지 + deltaTime + 클램프 + substep)**
- `src/ThrowMe/Physics/Vector2.cs`
- `src/ThrowMe/Physics/SlimePhysicsEngine.cs`
- `src/ThrowMe/Services/MonitorLayoutService.cs`
- `src/ThrowMe/Services/ThrowInputTracker.cs`
- `src/ThrowMe/Animation/SlimeAnimationController.cs` (기본 Squash/Stretch 골격)
...
```powershell
# 프로젝트 생성 후
cd C:\claudeProject\ThrowMe
dotnet build src/ThrowMe/ThrowMe.csproj -c Debug
dotnet run  --project src/ThrowMe/ThrowMe.csproj
```
...
- **실제 OS 입력 합성으로 실앱을 구동해 원인 규명**(외부 드라이버로 ThrowMe.exe 던지기·클릭):
...
- **테마별 커스텀 이미지(덧씌우기)** — 대상: **슬라임 · 당구공만**(볼 3종은 고유 디자인 유지, `SkinImageStore.Supports`).
  ...
  - `Services/SkinImageStore.cs` — `%APPDATA%/ThrowMe/skins/<스킨>.png`. 고른 원본을 **복사·PNG 재인코딩**해
