...
///
/// - <see cref="ServerBaseUrl"/> : 릴레이 서버 주소(예: wss://slimey-relay.<계정>.workers.dev).
/// - <see cref="RoomCode"/> + <see cref="Secret"/> : 같은 값을 입력한 PC 들이 한 방에 묶인다.
/// - <see cref="NodeId"/> : 이 PC의 표시 이름(기본 = 컴퓨터 이름). 방 안에서 고유해야 함.
///
/// 비밀값(시크릿)은 %LOCALAPPDATA%\Slimey\relay.json 에만 저장하고 리포지토리·배포 exe 에 넣지 않는다.
/// </summary>
public sealed class AuthService
{
    ...
    private static string ConfigDir =>
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Slimey");
    ...
    /// <summary>참여에 필요한 값이 모두 채워졌는가.</summary>
    ...
    ///   Slimey.exe --profile=A --node=PC-A --server=wss://... --room=TEST-1 --secret=pw --link=Right:PC-B:Left
